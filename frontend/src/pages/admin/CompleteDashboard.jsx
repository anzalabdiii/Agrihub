...
import {
  ...
  UserMinus, UserCheck, Trash2, Edit, Plus
} from 'lucide-react';
import toast from 'react-hot-toast';

export default function AdminCompleteDashboard() {
    ...
    <div>
      ...
      <div className="flex gap-4 mb-6">
        <button
          ...
          className={`btn ${view === 'farmers' ? 'btn-primary' : 'btn-secondary'}`}
        >
          Farmers ({farmers.length})
        </button>
        <button
          onClick={() => setView('buyers')}
          className={`btn ${view === 'buyers' ? 'btn-primary' : 'btn-secondary'}`}
        >
...
function PendingProductsTab() {
  ...
  if (products.length === 0) {
    return <div className="text-center py-8 text-gray-500">No pending products</div>;
  }
        ...
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="text-sm text-gray-600 mb-2">Quantity: {product.quantity} {product.unit}</p>
              <p className="text-sm text-gray-500 mb-4 line-clamp-2">{product.description}</p>
              <p className="text-xs text-gray-500 mb-4">Posted by: {product.farmer?.full_name}</p>
...
function PendingOrdersTab() {
  ...
  const approveOrder = async (orderId) => {
    try {
      ...
      toast.success('Order approved! Stock has been deducted.');
      loadOrders();
    } catch (error) {
  ...
  if (orders.length === 0) {
    return <div className="text-center py-8 text-gray-500">No pending orders</div>;
  }
          ...
          <div key={order.id} className="bg-white rounded-lg shadow p-6">
            ...
            {order.buyer_notes && (
              <div className="bg-gray-50 p-3 rounded mb-4">
                <p className="text-sm text-gray-600"><strong>Buyer Notes:</strong> {order.buyer_notes}</p>
              </div>
            )}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {logs.map((log) => (
              <tr key={log.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  {log.user_email} <span className="text-gray-500">({log.user_role})</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">
